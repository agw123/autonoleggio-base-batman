...
public class Noleggio {
	...
	public void aggiungiBatMobile(BatMobile batMobile) throws FileNotFoundException, IOException {
		...
		listaBatmobili = getBatmobileFile("listaBatmobili.txt");
		
		int sovrapposto=0;
		for (BatMobile bat:listaBatmobili)
		{
			if (bat.getIdAuto()==batMobile.getIdAuto())
			{
				sovrapposto=1;
				System.out.println("C'è conflitto di ID: \n l'ID immesso è già presente tra gli IDs delle Batmobili");
			}
		}
		if (sovrapposto==0)
		{
        ...
        if (!listaBatmobili.isEmpty()) {
			for (BatMobile bat : listaBatmobili) {
				if (bat.getIdAuto()==IDbatMobile){
					...
					riscriveBatmobileFile("listaBatmobili.txt");
	...
	public void aggiungiAuto(Auto auto) throws FileNotFoundException, IOException {
		autoDisponibili = getAutoFile("listaAuto.txt");
		int sovrapposto=0;
		for (Auto bat:autoDisponibili)
		{
			if (bat.getIdAuto()==auto.getIdAuto())
			{
				sovrapposto=1;
				System.out.println("C'è conflitto di ID: \n l'ID immesso è già presente tra gli IDs delle Automobili");
			}
		}
		if (sovrapposto==0)
		{
        ...
        if (!autoDisponibili.isEmpty()) {			
			for (Auto auto : autoDisponibili) {
				if (auto.getIdAuto() == idAuto){
					...
					riscriveAutoFile("listaAuto.txt");
	...
	public void aggiungiBatmobileFile(BatMobile batmobile) throws FileNotFoundException, IOException {
		String filePath = "listaBatmobili.txt";
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath, true))) {
